...
def get_blockchain():
    return requests.get(f'http://{BASE_URL}/blockchain').json()

def get_blockchain_mine():
    return requests.get(f'http://{BASE_URL}/blockchain/mine').json()

def post_wallet_transact(recipient, amount):
    return requests.post(
        f'http://{BASE_URL}/wallet/transact',
        json={'recipient': recipient, 'amount': amount}
    ).json()

def get_wallet_info():
    return requests.get(f'http://{BASE_URL}/wallet/info').json()
